...
class OpenAIServingChat(OpenAIServing):
    ...
    async def create_chat_completion(
        ...
        result = await self.render_chat_request(request) # 这里进行的tokenize
        ...
        for i, engine_input in enumerate(engine_inputs):
            ...
            # ===== engine_input = {'type': 'token', 'prompt_token_ids': [151644, 872, 198, 100057, 36589, 100434, 100113, 3837, 99208, 69249, 114880, 117196, 9370, 100475, 99662, 3837, 99984, 57811, 105934, 100027,
            # 116301, 104668, 1773, 34859, 14777, 1773, 151645, 198, 151644, 77091, 198], 'prompt': '<|im_start|>user\n秋意渐浓，风里藏着桂花的甜香，漫山枫叶如火，铺就一地斑斓。阳光透过稀疏的枝叶，洒下斑驳光影，温柔了岁月。
            # 每一缕风，都在诉说着岁月的故事。在这温柔的季节里，心也跟着沉静下来，仿佛能听见时光流淌的声音。请求一。
            # <|im_end|>\n<|im_start|>assistant\n', 'arrival_time': 1782466357.9022548, 'session_id': 'sub-1', 'parent_session_id': 'main-0', 'ttl': 300.0}
            logger.warning(f'===== engine_input = {engine_input}')
            ...
            logger.warning(f'===== sub_request_id = {sub_request_id}')
            ...
            if isinstance(sampling_params, BeamSearchParams):
                ...
                # ===== self.engine_client = <vllm.v1.engine.async_llm.AsyncLLM object at 0xfffb929fbfd0>
                logger.warning(f'===== self.engine_client = {self.engine_client}')

                generator = self.engine_client.generate( # todo 封装一个EngineCoreRequest对象
                    engine_input,
                    sampling_params,
                    sub_request_id,
                    lora_request=lora_request,
        ...
        logger.warning(f'===== self.chat_completion_full_generator') # todo 走这里
    ...
    async def chat_completion_full_generator(
        ...
        try:
            # ===== chat_completion_full_generator, session_id = sub-1, parent_session_id = main-0, ttl = 300.0
            logger.warning(f"===== chat_completion_full_generator, session_id = {request.agent_hint.session_id}, parent_session_id = {request.agent_hint.parent_session_id}, ttl = {request.agent_hint.cache_control.ttl}")
            async for res in result_generator:
                # ===== chat_completion_full_generator, res = RequestOutput(request_id=chatcmpl-abf515177b51be07, prompt='<|im_start|>user\n秋意渐浓，风里藏着桂花的甜香...
                logger.warning(f"===== chat_completion_full_generator, res = {res}")
                final_res = res
